...
def update():
    ...
    #
    # If the ball bounces off the left of the bat, it bounces towards the left
    # of the screen; if off the right of the bat, towards the right
    #
    left_bat = Bat(bat.topleft, (bat.width / 2, bat.height))
    right_bat = Bat((bat.centerx, bat.top), (bat.width / 2, bat.height))
    if ball.colliderect(left_bat):
        ball.direction = (dx if dx < 0 else -dx), -dy
    elif ball.colliderect(right_bat):
        ball.direction = (dx if dx > 0 else -dx), -dy
